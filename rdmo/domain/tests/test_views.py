...
export_formats = ('xml', 'html', 'rtf', 'csv', 'csvcomma')
...
@pytest.mark.parametrize('username,password', users)
def test_domain_import_post(db, client, username, password):
    client.login(username=username, password=password)

    url = reverse('domain_import', args=['xml'])
    with open('testing/xml/domain.xml', encoding='utf8') as f:
...
@pytest.mark.parametrize('username,password', users)
def test_domain_import_error_post(db, client, username, password):
    client.login(username=username, password=password)
