...
@pytest.mark.parametrize('username,password', users)
def test_questions_import_post(db, client, username, password):
    client.login(username=username, password=password)

    url = reverse('questions_catalog_import', args=['xml'])
    with open('testing/xml/questions.xml', encoding='utf8') as f:
...
@pytest.mark.parametrize('username,password', users)
def test_questions_import_error_post(db, client, username, password):
    client.login(username=username, password=password)
