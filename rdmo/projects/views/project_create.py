...
class ProjectCreateView(ObjectPermissionMixin, LoginRequiredMixin,
    ...
    def form_valid(self, form):
        ...
        if not settings.PROJECT_TASKS_SYNC:
            tasks = filter_tasks_or_views_for_project(Task, form.instance, user=self.request.user)
            for task in tasks:
                form.instance.tasks.add(task)

        # add all views to project
        if not settings.PROJECT_VIEWS_SYNC:
            views = filter_tasks_or_views_for_project(View, form.instance, user=self.request.user)
            for view in views:
